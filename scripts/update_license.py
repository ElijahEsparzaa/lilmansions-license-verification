...
DRE_LOOKUP_URL = (
    f"https://www2.dre.ca.gov/PublicASP/pplinfo.asp?License_id={LICENSE_ID}"
)
...
def find_license_download_url():
    ...
    raise RuntimeError(
        "Could not find the official DRE 'Licensee List ZIP' download link."
    )
...
def header_score(row):
    ...
    if any(
        "licenseid" in v or v in {"license", "licenseid"}
        for v in vals
    ):
        score += 4

    if any(
        "expiration" in v or "expirationdate" in v
        for v in vals
    ):
        score += 2

    if any(
        "licensestatus" in v or v == "status"
        for v in vals
    ):
        score += 2
...
def find_record_in_dataframe(df):
    ...
    if header_row is not None:
        ...
        for col in license_cols:
            matches = data[
                data[col].map(norm) == norm(LICENSE_ID)
            ]
...
def find_record_in_zip(zip_bytes):
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as z:
        ...
        if not names:
            raise RuntimeError(
                "The DRE ZIP did not contain an Excel/CSV data file."
            )
        ...
        for name in names:
            ...
            # CSV files from the DRE may not be UTF-8 encoded.
...
def main():
    print(
        f"Updating California DRE license {LICENSE_ID}"
    )

    download_url = find_license_download_url()

    print(
        f"Official DRE download URL: {download_url}"
    )
    ...
    if not record:
        raise RuntimeError(
            f"License {LICENSE_ID} was not found "
            "in the official DRE Licensee List."
        )
